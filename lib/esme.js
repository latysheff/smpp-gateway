...
class ESME extends EventEmitter {
  constructor (options) {
    ...
    if (webhook) {
      this.webhookUrl = new url.URL(webhook).toString()
    }
  ...
  connect () {
    ...
    this.session.on('connect', async () => {
      ...
      // confirm deliver_sm
      this.session.on('deliver_sm', this.acceptMessage.bind(this))

      // confirm data_sm
      this.session.on('data_sm', this.acceptMessage.bind(this))
  ...
  webhook (pdu) {
    ...
    setImmediate(async () => {
      debug('execute webhook to', this.webhookUrl)
      // todo user friendly JSON message
      try {
        await request.post({
          url: this.webhookUrl,
          json: { pdu }
